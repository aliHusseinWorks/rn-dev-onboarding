...
- A section's "Copy all" dropped its last command: the block ended without a newline, so pasting 15 install commands ran 14 and left the last sitting at the prompt — a partial install that reads as a complete one. Same root cause as the scan script's missing newline, which only got fixed on the scan script.
