...
export const TOOLS: Tool[] = [
  ...
  },
  {
    ...
    modal: {
      ...
      prereq: 'A Zoho account on a plan that includes Zoho MCP (licensed separately — your Zoho admin can confirm).',
      fields: [
        { key: 'name', label: 'Name this server', placeholder: 'zoho-cliq' },
        { key: 'mcp-url', label: 'Its URL, from the Connect tab', placeholder: 'https://…zohomcp.com/mcp/…/message' },
      ],
      steps: [
        {
          command: 'mcp.zoho.com → Create MCP server → one app → tick its tools → Connect tab → copy the URL',
          note: 'Create MCP server, not a pre-configured one — the pre-configured "Cliq Messaging" connects and then exposes zero tools. Match your data centre (mcp.zoho.eu / .in / .com.au).',
          manual: true,
        ...
        },
        {
          ...
          note: 'Then approve OAuth. Adding another app? Change both boxes above and copy this again.',
          shellQuoted: true,
        },
        {
          command: 'Restart Claude Code.',
          note: 'And again whenever you change which tools are ticked — a client reads the tool list when it connects, so a running session keeps the old one. The OAuth grant itself survives.',
          manual: true,
        },
      ],
    },
